...
if (file_exists("cfg.ini")) {
	...
	try {
		$bdd = new PDO('mysql:host=' . htmlspecialchars($dbi['dbsrvname']) .
			';dbname=' . $dbi['dbname'] .
			';charset=utf8',
			htmlspecialchars($dbi['dbsrvname']),
			htmlspecialchars($dbi['dbpasswd']));
		if (file_exists("database.sql")) {
			$req = run_sql_file("database.sql", $bdd);
		} else {
			$error = 1;
		}
	} catch (Exception $e) {
	...
	<body>
		<div id="page">
			...
			<?php if (($req['total'] - $req['success']) !== 0) { ?> 
				<p>ATTENTION : la base de données n'a pas pu être totalement mise en place. <br /> <strong><?php echo ($req['total'] - $req['success']); ?></strong> instructions n'ont pas été correctement exécutée(s).</p>
