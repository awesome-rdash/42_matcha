...
class MemberManager {
	...
	public function update(Member $member) {
		$q = $this->_db->prepare('
			UPDATE users
			SET nickname = :nickname, email = :email, password = :password, register_time = :register_time, birthdate = :birthdate, lastname = :lastname, firstname  = :firstname, phone = :phone, sexe = :sexe, bio = :bio, mail_confirmed = :mail_confirmed, sexual_orientation = :sexual_orientation, featuredPictures = :featuredPictures, profilePicture = :profilePicture, lastLogin = :lastLogin, popularity = :popularity, locationLat = :locLat, locationLong = :locLong
			WHERE id = :id');
		...
		$q->bindValue(':locLong', $member->getLocationLong());
		$q->bindValue(':locLat', $member->getLocationLat());
